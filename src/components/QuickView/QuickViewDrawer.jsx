...
import { FiChevronLeft, FiChevronRight } from 'react-icons/fi'; // Icons for main image navigation

const QuickViewDrawer = ({ product, onClose }) => {
  const [quantity, setQuantity] = useState(1);
  const [activeImgIndex, setActiveImgIndex] = useState(0); // Track by index for easier arrow navigation
  ...
  return (
    ...
    >
      <div className="max-w-[1300px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 p-6 md:p-16 relative">
        ...
        {/* LEFT SECTION: Image Gallery with Navigation (Matches Image 5) */}
        <div className="flex flex-col space-y-4">
          <div className="relative w-full aspect-[3/4] bg-white group flex items-center justify-center overflow-hidden">
            ...
            {/* Main Image Navigation Arrows (Feature requested) */}
            <button 
              onClick={prevImg}
              className="absolute left-0 top-1/2 -translate-y-1/2 bg-white/40 p-3 hover:bg-white/70 transition-colors z-10"
            >
              <FiChevronLeft size={24} className="text-gray-800" />
            </button>
            <button 
              onClick={nextImg}
              className="absolute right-0 top-1/2 -translate-y-1/2 bg-white/40 p-3 hover:bg-white/70 transition-colors z-10"
            >
              <FiChevronRight size={24} className="text-gray-800" />
            </button>
          </div>
          
          {/* Thumbnail Gallery (Matches Image 5) */}
        ...
        {/* RIGHT SECTION: Product Controls (Matches Image 1) */}
        <div className="flex flex-col text-left space-y-5">
          <header className="space-y-2">
            <h2 className="text-3xl font-serif tracking-tight text-[#1a1a1a]">{product.name}</h2>
          ...
          <div className="space-y-6 pt-2">
            <div>
              ...
              <div className="border border-gray-400 px-5 py-2 text-sm w-fit bg-white">Pastel Mint</div>
            </div>
            ...
            {/* Quantity Selector (Matches Image 1) */}
            <div>
          ...
          <button className="text-[11px] underline text-gray-500 tracking-widest hover:text-black uppercase mt-4 text-left">
            More details &gt;
          </button>
        </div>
      </div>
